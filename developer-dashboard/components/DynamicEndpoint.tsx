...
import type { Endpoint } from '@/services/apiDocumentationService';

interface DynamicEndpointProps {
  endpoint?: Endpoint;
  category?: string;
}

const DynamicEndpoint: React.FC<DynamicEndpointProps> = ({ endpoint, category }) => {
  ...
  const SyntaxHighlightedJSON: React.FC<{ code: string }> = ({ code }) => {
  ...
  const generateCURL = (method: string, url: string, requestPayload?: string): string => {
  ...
  const EndpointCard: React.FC<EndpointCardProps> = ({ 
    ...
    const [viewMode, setViewMode] = useState<'request' | 'response' | 'curl'>('request');
      ...
      <div className="p-5 space-y-4">
        <div>
          <p className="text-sm text-slate-300 leading-relaxed">
            {endpoint?.overview?.description || 'API endpoint documentation'}
          </p>
        </div>
  ...
  if (!endpoint) {
    return (
      <div className="border border-slate-700/50 rounded-lg p-8 bg-slate-900/30">
        <p className="text-slate-400 text-center">No endpoint data available</p>
      </div>
    );
  }
      ...
      {endpoint.methods.map((methodData, idx) => (
        <div key={methodData.id || idx} id={methodData.id} className="scroll-mt-20">
          <EndpointCard
            ...
            methodId={methodData.id || String(idx)}
          />
        </div>
      ))}
    </div>
