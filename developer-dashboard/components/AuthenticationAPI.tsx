...
const AuthenticationAPI: React.FC = () => {
  ...
  const EndpointCard: React.FC<EndpointCardProps> = ({ method, url, title, description, requestPayload, responsePayload, id: cardId }) => (
    ...
    return (
      <div className="mt-4 border border-slate-700/50 rounded-lg overflow-hidden bg-slate-900/30">
        ...
        <div className="bg-slate-950/50">
          ...
          <div className="p-4 overflow-x-auto">
            <SyntaxHighlightedCode code={codeExamples[activeTab]} language={tabs.find(t => t.id === activeTab)?.lang || 'javascript'} />
          </div>
        </div>
      </div>
    );
